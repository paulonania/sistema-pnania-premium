...
<span style="background-color: {color_hex}; color: #ffffff; font-size: 11px; font-weight: 700; padding: 3px 8px; border-radius: 12px; text-transform: uppercase; letter-spacing: 0.5px;">{classificacao}</span>
{ideal_html}
</div>
</div>"""
...
def render_resumo_cards(stats, meta, df):
    ...
    with st.container(border=True):
        ...
        with col_gen[1]:
            ...
            rotulo_perfil, cor_perfil = classificar_perfil(media_fases)
            st.markdown(
    ...
    # --- Bloco 2: Consistência e Regularidade ---
    with st.container(border=True):
        st.markdown("<h4 style='text-align: center; color: #0f3a61; margin-top: 5px; margin-bottom: 15px;'>Consistência e Regularidade</h4>", unsafe_allow_html=True)
...
def render_diagnostico():
    ...
    pdf_bytes = gerar_pdf(meta=meta, fig_penetro=fig1, fig_espessura=fig2, fig_umidade=fig3)
    ...
    if st.button("Gerar Comparativo", type="primary", use_container_width=True) or st.session_state.get("comp_ativo"):
        if st.session_state["comparativo_lista"]:
            ...
            for item in st.session_state["comparativo_lista"]:
                ...
                rotulo_perfil, _ = classificar_perfil(media_fases)
            ...
            fig_comp = fig_comparativa(st.session_state["comparativo_lista"])
            st.pyplot(fig_comp)
        else:
            st.session_state["comp_ativo"] = False
...
def render_relatorio_pdf():
    ...
    pdf_bytes = gerar_pdf(meta=meta, fig_penetro=None, apenas_tabelas=True)
    ...
    with col_download:
        st.download_button(
            "📥 Exportar Laudo de Diagnóstico (PDF)",
            pdf_bytes,
            file_name=f"{nome_arquivo('Relatorio_Diagnostico', meta)}.pdf",
            mime="application/pdf",
            type="primary",
            use_container_width=True
        )
    ...
    # Legenda explicativa de CV%
    st.markdown("""
    <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px 16px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-size: 12px; color: #475569;">
        <strong style="color: #1e293b;">Legenda de Classificação de Consistência (CV%):</strong>
        <ul style="margin: 6px 0 0 0; padding-left: 20px; line-height: 1.5;">
            <li><strong>EXCELENTE:</strong> CV &lt; 5%</li>
            <li><strong>BOA:</strong> 5% &le; CV &lt; 10%</li>
            <li><strong>SATISFATÓRIA:</strong> 10% &le; CV &lt; 15%</li>
            <li><strong>CRÍTICA:</strong> CV &ge; 15%</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)

    # Parecer Técnico e Notas
